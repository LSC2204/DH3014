...
class MeanScaleUniformBins(ChronosTokenizer):
    ...
    def input_transform(
        ...
        scale[~(scale > 0)] = 1.0
        scaled_context = context / scale.unsqueeze(dim=-1)
        self.boundaries = self.boundaries.to(scaled_context.device)  # Move boundaries tensor to the same device as scaled_context
        token_ids = (
            torch.bucketize(
                input=scaled_context,
                boundaries=self.boundaries,
    ...
    def output_transform(
        ...
        scale_unsqueezed = scale.unsqueeze(-1).unsqueeze(-1).to(self.centers.device) 
        indices = torch.clamp(
            samples - self.config.n_special_tokens,
            min=0,
            max=len(self.centers) - 1,
